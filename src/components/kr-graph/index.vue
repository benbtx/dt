...
import zdry from "@/assets/image/zdry.png";
import zddh from "@/assets/image/zddh.png";
import zdyhk from "@/assets/image/zdyhk.png";
export default {
    props: {
        selectOptions: {
            type: Object,
    ...
    methods: {
            ...
            if (this.model == "circle") {
                ...
                if (this.model == "pathRelation") {
                    for (let n = 0; n < 1; n++) {
                        let x =
                            this.offsetX +
                            ((this.width - 2 * this.offsetX) / 2) * (n + 1);
                        nodes.push({
                            x,
                            y: this.height / 2,
                            fx: x,
                ...
                if (this.model == "pathRelation") {
                    this.force.force("charge", d3.forceCollide(100));
                }
                if (this.model == "graph") {
                    this.force.force(
                        "center",
        ...
        createNodeAndLine() {
            ...
            enterNode
                .on("mouseenter", currNode => {
                    if (this.isClick && this.isClickHighlight) {
                        return;
                    }
                    ...
                    if (this.isAddLine) {
                        ...
                        if (this.addLineNodes.length == 2) {
                            ...
                            this.linksData.map(value => {
                                if (value.type == "add") {
                                    if (
                                        (value.source.id == line.source &&
                                            value.target.id == line.target) ||
                                        (value.source.id == line.target &&
                                            value.target.id == line.source)
                                    ) {
                                        isHave = true;
                                    }
                                }
        ...
        getLinkPath(link) {
            ...
            if (link.type == "add") {
                ...
                this.linksData.map(value => {
                    if (value.type != "add") {
                        if (
                            (value.source.id == link.source.id &&
                                value.target.id == link.target.id) ||
                            (value.source.id == link.target.id &&
                                value.target.id == link.source.id)
                        ) {
                            isHave = true;
                        }
                    }
                ...
                if (isHave) {
                    link.bezier = this.bezierDistance;
                    let bezier = this.getBezier(link);
                    return (
                        "M" +
                        sx +
                        " " +
        ...
        findSelectNode(start, end) {
            ...
            if (hideList.length > 0) {
                this.deleteNodes(hideList, [], false);
            }
        },
        ...
        getImg(node) {
            let ob = "";
            if (node.isImportant) {
                return this.importantImgs[node.nodeType]
                    ? this.importantImgs[node.nodeType]
                    : zdry;
            }
            if (node.objectType == "1") {
                ob = node.nodeType + "" + node.objectType;
        ...
        layoutCircle() {
            let r = 0,
                center = [this.width / 2 - 30, this.height / 2 - 30],
                angle = (2 * Math.PI) / this.nodesData.length;
            if (this.height > this.width) {
                r = (this.width - 120) / 2;
            } else {
                r = (this.height - 120) / 2;
            }
            this.nodesData.map((value, index) => {
                value.x = center[0] + r * Math.cos(angle * index);
                value.y = center[1] + r * Math.sin(angle * index);
            });
        },
        getLeft(e) {
