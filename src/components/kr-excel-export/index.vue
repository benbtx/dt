...
export default {
    ...
    methods: {
        handleClick() {
            ...
            let exportConfig = [];
            let configType = getType(this.exportConfig);
            switch (configType) {
                case "Function":
            ...
            let syncGetDatas = exportConfig.map(item => {
        ...
        solveData2Sheet(exportConfig) {
            ...
            exportConfig.forEach(function(vm, index) {
                let { config, resData, sheetName = `sheet${index + 1}` } = vm;
                let { includes, formatters } = config;
