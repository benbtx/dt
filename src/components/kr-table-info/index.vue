...
export default {
    ...
    filters: {
        computedLenth(cell = {}) {
            let width = (cell.label.length + 1) * 14 + "px";
            return { width, minWidth: width };
        }
    },
    computed: {
        computedCells() {
            let { cells, cellsPerLine, data } = this;
            let lines = [];
            let solvedCells = [];
            cells.forEach(cell => {
                let { colspan = 1 } = cell;
                if (!colspan) {
                    ...
                    let arr = Array.from({ length: colspan + 1 }).fill({});
                    arr[0] = cell;
                    solvedCells = solvedCells.concat(...arr);
                }
            });
            for (
                let i = 0, len = solvedCells.length;
                i < len;
                i += cellsPerLine
            ) {
                let arr = [];
